...
export default function SolutionImageBanner({ slug, name, icon: Icon }) {
  const { language } = useLanguage()
  const imageData = getSolutionImage(slug)
  ...
  return (
    <div
      className="relative flex h-72 sm:h-96 lg:h-[420px] w-full items-end justify-between overflow-hidden rounded-card border-2 border-white/80 bg-slate-800 p-6 sm:p-10 shadow-lifted group"
      role="img"
      aria-label={name}
    >
      {/* Ultra-Clear, Bright, Vibrant High-Resolution Photography */}
      <img
        src={imageData.url}
        alt={subtitleText}
